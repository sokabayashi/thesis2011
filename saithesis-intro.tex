...
& 	\textbullet \, Converges rapidly when it   	& \textbullet \, May not converge depending on starting\\ 
& 	converges 	& point \\				
&				& \textbullet \, Requires second derivatives\\[1ex]
\hline
...
may not exist.  
In such a case, the MLE is actually off ``at infinity" in some direction in the parameter space, called a \emph{direction of recession}.
...
does \textbf{[MUST?]} exist and can be found using conventional methods such as logistic regression.  
...
\citeauthor{Rinaldo:2009} show that the normal fans of this support 
...
theory of exponential families.  The perceived pathology then, is a result of 
model specification rather than any pernicious theoretical flaw.
...
model and thus avoid the degenerate behavior of points 3 and 4. 
...
When it cannot, it still has a particularly convenient form that is 
...
In the case for non-existent MLEs, our algorithm returns the information necessary
...
a dark truth of exponential family models with complex dependence is that 
whether or not the MLE exists cannot be known in advance of the search effort. 
In \citet{Handcock:degeneracy,Rinaldo:2009}, the authors work with full knowledge of 
the convex support of their examples, something only attainable if one is able to
evaluate the likelihood.  While they dissect these particular examples 
...
the Fisher information matrix becomes useful.  
...
Our line search algorithm with $p_k$ 
...
guaranteed to exist.
Our algorithm looks to find this new maximizer.  In order to do so, it must 
recognize that the objective function it needs to maximize has changed.
