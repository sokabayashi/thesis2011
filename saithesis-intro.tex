...
In the context of epidemiology, a network modeling  
...
and observed that the MPLEs are close to those they arrived at from their rigorous simulations, leading them to conclude that these MPLEs are acceptable.  
...
\hl{ We discuss one particular} \citep{Hummel}. 
...
\subsection{Steplength MCMC-MLE}
...
for dealing with non-existent MLEs.  
We prove convergence in the case where the MLE exists and the first 
...
In the case where the MLE may not exist in the conventional sense, 
our algorithm returns the information necessary
to construct one-sided confidence intervals for the MLEs, which are actually off 
``at infinity".  Our algorithm requires little modification; in fact,
...
As discussed in Section~\ref{S:LCM}, $\ell(\eta)$ is still bounded even if
