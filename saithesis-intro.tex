...
themselves.  This requires obtaining the maximum likelihood distribution which
...
particular emphasis on the sensibleness of the distributions generated from the specified models.  
...
for our computational approach for counting simple statistics in the 9-node network
...
\citet{Hummel} raise two additional issues with the pseudolikelihood approach
(which also apply to the composite likelihood approach)
in the context of network models. The first is that this approach requires
...
which we would like to fit an ERGM.  Thus given a $g(\yobs)$, one would then
...
will yield the same $g(\yobs)$.  Because ERGMs only depend on $g(\yobs)$,
...
platform now overwhelmingly use MLE methods, using MPLE only as a starting point.
...
approximation by \eqref{E:r_hat} using 100,000 MCMC samples.  Data for plots are generated using code accompanying \citet{Hummel}.} 
\label{F:MCMC-MLE}
\end{figure} 
...
\hl{CHARLIE SAYS ADD PENTTINEN.}  \citep{Penttinen:1984}
...
and other models with complex dependence.
...
We discuss in Chapter~\ref{Chapter:Linear programming} the adjustments that need to 
