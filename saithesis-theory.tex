...
A \emph{convex polytope} $C$ is the \emph{convex hull} of a finite set of points $V$,
\begin{align*}
	C = \con( V ).
\end{align*}
...
\emph{if $C$ is known} using linear programming.
...
(\, \cdot\,  | g(y) \in H)$.  This is because the numerator
...
Then the conditional density for $Y|[g(Y)\in H]$ can be expressed
\begin{align*}
...
The log likelihood for $f_\eta( \, \cdot\,  | g(y) \in H)$ for $\eta \in \Xi$ can be expressed as
\begin{align*}
%	\ell_{LCM}(\eta) &= 
...
An MLE for the LCM $\etaLCM$ in turn must exist when the convex support $C$ is polyhedral 
...
it then $g(\yobs)$ must lie on the boundary of $C$.  The LCM is the distribution
...
boundary is actually the face is $C \cap H$ on which $g(\yobs)$ sits.
