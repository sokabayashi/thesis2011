...
\hl{Define the \emph{convex hull} of a set of points $V$, denoted $\con V$, to be}
\begin{align*}
	\left \{ \sum_{i} \alpha_i v_i \mid \alpha_i \in \RR, \alpha_i \geq 0, \sum_i \alpha_i = 1 \right \} .
\end{align*}
The \emph{positive hull} of a set of points $V$, denoted $\pos V$, to be
\begin{align*}
	\left \{ \sum_{i} \alpha_i v_i \mid \alpha_i \in \RR, \alpha_i \geq 0 \right \} .
\end{align*}

A \emph{convex polytope} $C$ is the convex hull of a \emph{finite} set of points $V$.
...
This is illustrated in Figure~\ref{F:LCM} for the case of a two-dimensional parameter
space.
...
\centering
	...
	the direction $\delta$, converging to the LCM log likelihood for a two-dimensional parameter space.  The log likelihood for $\ell(\etaLCM + s\delta)$
