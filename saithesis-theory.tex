...
\begin{align*}
	\mu = \E_\eta g(Y).
\end{align*}
This maps any $\eta \in \Xi$ for which the expectation exists to a point in
the relative interior of the convex support.
Thus mean value parameters are more easily interpretable quantity, since
they reside in the same space as the natural statistics 
\citep{Handcock:degeneracy,Rinaldo:2009}.
In particular, $\mu = g(\yobs)$ is the MLE in the mean value parameterization,
as established by \eqref{E:Observed-Expected}.  
...
more structure in this context---it is $\etaLCM$ sent to infinity in direction $\delta$.

We may be interested in knowing how close the natural parameter $\eta$ is to infinity.
As $s$ goes from $-\infty$ to $+\infty$, the probability of observing $g(\yobs)$
goes from zero to one in a strictly increasing manner.  This event of observing
$g(\yobs)$ corresponds exactly to the event $g(Y) \in H$, and thus we look to find the
unique $s$, call it $\hat{s}$, that makes this probability $\alpha$.  
That is, we find the unique $s$ such that
\begin{align*}
...
a $1-\alpha$ confidence region then for the parameter $\etaLCM + s \delta$, which we can
think of as a confidence region for how close the natural parameter is to infinity.
