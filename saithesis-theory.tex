...
Wwhen $\delta$ is a direction of recession that is not a direction of constnacy, 
...
perspective.  We will discuss it more detail in Chapter~\ref{Chapter:Linear programming}.
...
Left to show is that there is no smaller set than $C \cap H$ can be convex support for the LCM.
%Let $S = g(\YY)$, and define $A = \con( S \cap H )$.  Does $A = C \cap H )$?
%Clearly $ A \subset (C \cap H)$.  But is there an $x \in C\cap H$ where $x \notin A$?
Let $S = g(\YY)$.  Clearly, $(S \cap H) \subset (C \cap H)$.  Is $(C \cap H) = \con(S \cap H)$?
If a point $x \in C$, it is a convex combination of finite points in $S$, which we may divide up
into points in $S \setminus H$ and $S \cap H$.  
\begin{align*}
	x = \alpha_1 y_1 + \alpha_2 y_2 + \cdots + \alpha_k y_k + \alpha_{k+1} y_{k+1} + \cdots + \alpha_n y_n
\end{align*}
where $\alpha_i \geq 0$, $\sum_i \alpha_i = 1$, and we designate $y_1, \ldots, y_k \in S \setminus H$
and $y_{k+1}, \ldots, y_n \in S \cap H$.

For any $x \in C \cap H$, the coefficients of the points not in $S \cap H$ must all be zero,
otherwise there will be a contribution from a component puts the point outside $H$.
Then $x$ will equal a convex combination of just the points in $S \cap H$.  Since this is 
true for any $x \in C \cap H$,  $C \cap H = \con( S\cap H)$.
\end{proof}
