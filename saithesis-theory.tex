...
The implications of Theorem~\ref{Thm:LCM} are far reaching.
The right-hand side of \eqref{E:LCM} can be viewed as a conditional density of a 
distribution 
\textbf{[WHY?]} 
with parameter $\eta$ given $g(Y) \in H$ and hence expressed as $f_{\eta}
...
original family.  WHY A CONDITIONAL DENSITY?
...
The log likelihood for this can be expressed as
\begin{align*}
	\ell_{LCM}(\eta) %&= \inner{\eta, g(\yobs)} - c(\eta) - \log P_\eta(g(Y) \in H) \\
...
\end{align*}
\textbf{[STRICTLY $<$?]}
Thus even though the MLE may not exist for the original family, its log likelihood
$\ell(\eta)$ is bounded by the LCM log likelihood for which a maximizer will exist.
\textbf{[REALLY?  MLE OF LCM MUST EXIST?]}
...
The last part of 
this theorem says that for the sequence of exponential families with parameter 
