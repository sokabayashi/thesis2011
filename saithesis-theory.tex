...
\begin{align*}
	\mu = \E_\eta g(Y).
\end{align*}
Thus mean value parameters are the expectation of a more easily interpretable quantity
\citep{Handcock:degeneracy, Rinaldo:2009}.
The space for the mean value parameterization is the relative interior of the 
convex support of the natural statistics.  
...
While the above theorem is concise, we note that in order to apply this result, the geometry of $C$ must be fully known in order for it to be useful.
...
$f_\eta( \fatdot \mid g(y) \in H)$, $\eta \in \Xi$ is still the 
...
When we find the MLE in the LCM, we say we have found an MLE in the Barndorff-Nielsen completion of the original family.
...
%union of the LCM and the original family.  
\hl{DEFINE $\Gammalim$.}
