...
\State Get an initial value, $\eta_1$.  The observed data, $\yobs$, is of course known.
\State \textbf{Sample} $Y_1, \ldots, Y_m$ from the distribution at $\eta_{1}$.
\State Make a V-rep from this sample and reduce using \texttt{redundant}.  The result are the vertices of the convex hull. Store them in $ty.hull$.
...
\State Sample $Y_1, \ldots, Y_m$ from the distribution at $\eta_{k+1}$.
\State Make a V-rep from this sample and reduce using \texttt{redundant}.
...
\State \textbf{Question}: Is $\yobs$ outside the convex hull of the sample?
\If{Yes, $\yobs$ is outside the convex hull}
	\State  Keep sampling.  Skip to estimating $\nabla \ell( \eta_{k+1})$.
\ElsIf{No, $\yobs$ is inside the convex hull}
	\State Then the MLE exists, finding it should be straightforward.  Skip to
	\State estimating $\nabla \ell( \eta_{k+1})$.
\Else 
	...
	\State convex hull of the entire sample space.\\
	...
	\If{$face.prop < 10\%$}
		\State	Keep sampling!	Regardless of what case we're in.
	\Else
		...
		\If{only 1 point}
			\State We have either a single-point face. WHAT TO DO?
		\Else
...
\If{ $k \geq 12$ \textbf{and} $k \mod 3==0$ \textbf{and} $LCM.flag$==FALSE}
