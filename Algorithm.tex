...
\title{Algorithm for Maximizing the Likelihood of Exponential Families}
...
Rationale (according to a talk by Rinaldo): ``Degeneracy occurs when the probability mass is spread over a small number of network statistics, so degenerate distributions will tend to correspond to values of $\theta$ for which the entropy function is small."

Three comments
\begin{itemize}
\item Large portions of the natural parameter space corresponds to points of low entropy.
\item Entropy values can change by large amounts for points close together in the parameter space.
\item There is structure in the entropy plot that can be characterized by the directions of recession.
\end{itemize}

I actually am not sure what their result is; is it that non-existence of MLEs can be characterized in terms of the entropy plots?
...
\State Get an initial value, $\eta_1$.  The observed data, $\yobs$, is known.
\State \textbf{Sample} $Y_1, \ldots, Y_m$ from the distribution at $\eta_{1}$.
\State Make a V-rep from this sample and reduce using \texttt{unique} followed by \texttt{redundant}.  The result are the vertices of the convex hull. Store them in $ty.hull$.
\State \textbf{Estimate} $\nabla \ell( \eta_1)$ and $\lVert \nabla \ell( \eta_1) \rVert$ using the sample.\\
\State Set $p_1 = \nabla \ell( \eta_1)$. 
\State Set $k=2$, $LCM.k=1$, $LCM.flag$ = FALSE. 
...
\end{align*}
\State for some $0 < c < 1$.  
\Comment{CJG thinks simulated tempering will work best.}\\

\State $\eta_{k+1} = \eta_k + \alpha_k p_k$.
\State \textbf{Sample} $Y_1, \ldots, Y_m$ from the distribution at $\eta_{k+1}$.
\State Make a V-rep from this sample and reduce using \texttt{unique} and \texttt{redundant}.
\State Combine reduced sample with $ty.hull$ and apply \texttt{redundant} again.
\State Get \textbf{normal cone} of $ty.hull$.
\State Get \textbf{H-rep} from reduced sample by using \texttt{scdd}.\\

\State \textbf{Question}: Is $\yobs$ outside the convex hull of the sample?  Use H-rep.
\If{Yes, $\yobs$ is outside the convex hull}
	\State  Keep sampling.  
...
\ElsIf{No, $\yobs$ is inside the convex hull}
	...
	\State Skip to estimating $\nabla \ell( \eta_{k+1})$.
\Else 
	\State No, it's on the border.  In which case, either
	\State (1) the MLE exists but we got a sample that just touches $\yobs$, 
	\State (2) the MLE does not exist since $\yobs$ is itself on the border of the 
	\State convex hull of the \emph{entire} sample space.\\
...
\If{$LCM.flag$ == FALSE and $on.interior$ == FALSE and $k$ divisible by 5}
	\State Find a ncone direction from the normal cone by averaging.
	\State Set $p_k = $ ncone direction.
\ElsIf{$LCM.flag$ == FALSE and $k \geq 9$  and divisible by 3}
	\State Run regression on last 8 points.
	\State Set $p_k = $ regression slope.
\ElsIf{ $LCM.flag$ == TRUE and $LCM.k \geq 8$ and divisible by 4}
...
\section{Problematic interior points 9/9/10}
Points like $(21,4)$ are on the relative interior of the convex hull, barely.  Although maximizing the log likelihood when this point is the observed data can be a little difficult as described earlier, the MLE nonetheless must exist.  Using the \texttt{trust} package in R, we estimate the MLE to be $\hat{\theta}_{MLE} = (28.85685, -7.75672)$. 

Our new LCM algorithm has a particularly difficult time with this point as the observed data because there is a good chance it will determine that the point is on the boundary of the entire sample space, and thus the MLE will not exist!

Why does this happen?  

As the algorithm proceeds uphill on the likelihood function, it's possible that for a value of $\theta = (35.3408851 -9.556440)$, all 10,000 points from the MC sample will fall on the six integer-coordinate points on the line segment from $(20,0)$ to $(25,20)$.  According to such a sample, $(21,4)$ is on this line segment.  Therefore, according to the algorithm, these six points form a face on the boundary of the convex hull.

The algorithm will then proceed by maximizing the likelihood on this restricted set of the empirical face, which typically involves moving in a direction orthogonal to what it thought was a direction of recession in the original problem.  The parameter value that sets the expected value with respect to the distribution on this restricted sample space equal to the observed value of $(21,4)$ is the MLE of this LCM, which we will denote $\hat{\theta}_{LCM}$.  In this case, we calculate this to be $(35.3831787, -9.387266)$.  So, we have
...
which are quite different distributions!

But all is not lost, for two reasons:
\begin{enumerate}
\item The ``fake" LCM parameter estimate, when we account for a direction of constancy in this LCM, are actually very close to the MLE.
\item The ``fake" LCM parameter estimate, combined with the ``fake" LCM sample space, assign almost identical probabilities to the sample points as the MLE distribution!
\end{enumerate}

Pursuing (1) further, we know a direction of constancy for the LCM, $\gamma_{lim}$, is in fact the normal to the empirical face, so any vector in the direction of $(4,-1)$.  Then we claim that 
\begin{align*}
	\hat{\theta}_{LCM} + \gamma_{lim}\\
	= 	\hat{\theta}_{LCM} + k \, (4,-1)\\
\end{align*}
will actually be close to the MLE.  Of course, we don't know what magnitude to use, but if we had perfect knowledge and chose $k = -1.631582$ to match the edge coordinate exactly, we would get
\begin{align*}
	\hat{\theta}_{LCM} + k \, (4,-1)= (28.856852 -7.755684)
\end{align*}
where we see that the triangle parameter is in fact very close to that of the MLE.

For (2), if we compare what probabilities our ``fake" LCM distribution gives to the empirical face points and compare to the probabilities that the MLE would give to these points in the original model, we see that they are quite close!
...
The sum of the absolute value of differences is only $0.0031$ on the face points.  If we include the additional 0.001 probability on points that our fake LCM distribution excludes, that is a difference in probability of $0.0041$.  

So, we have done something completely wrong---we've gotten very different looking parameter estimates on a very different sample space.  And yet the distributions are remarkably similar!

MORAL OF THE STORY: BAD PARAMETERS BUT GOOD PROBABILITIES.

SO, how do the CI look?
...
ANOTHER IDEA: how about an additional sample after finding what we are certain is the face, just to see if the sample changes?  In this case, it's possible that we might ``recover" more points from the sample space, perhaps even ones that will correct our wrong conclusion of a boundary point, or even just to recover more points on the face.

\section{Three-dimensional sample space --- 9/18/10}
We can make our problem more interesting by adding another statistic---the number of two-stars---to our natural parameter vector.  DEFINE two-stars.

This should make the prospect of finding faces more interesting since now they can be zero, one, or two-dimensional.

\begin{itemize}
\item Count table of edges, triangles, two-stars.  Done and verified, 9/17/10.
\item 3D plot?  misc3d?
\item Calc and save MLE info to file.
\item convex hull, faces, functions for finding normal cones, etc.
\end{itemize}

It is tempting to just draw the whole polygon.  However, as I've already seen, the 3D picture might not actually be so useful for looking at the entire space if one of the dimensions is 3x larger than the others (two stars vs. triangles).  What we actually want is to zoom in on the surface of the polygon at a certain point.
...
like (21,4).  The algorithm needs to be generalized.  \textbf{Record some pictures!}
\item Turn on MC for estimation of gradient.  9/12/10.  not too bad!  appears to work just fine.
\item MCMC?  Can we use the ergm sampler?  This might be terrible.  In fact, it might be worth conducting a study of how much the MCMC sample differs from the perfect MC sampler.
