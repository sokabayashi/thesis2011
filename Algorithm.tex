...
\ElsIf{No, $\yobs$ is inside the convex hull}
	\State The MLE exists, finding it should be straightforward.  Skip to
	\State estimating $\nabla \ell( \eta_{k+1})$.
\Else 
...
\If{$LCM.flag$ == FALSE and $k$ large enough and divisible by 3}
	\State Run regression on last 9 points.
	\State Set $p_k = $ regression slope.
\ElsIf{$LCM.flag$ == TRUE}
	\If{ $LCM.k$ is 1 or divisible by 3}
		\State Set $p_k = $ normal cone direction.
	\ElsIf{ $LCM.k$ is large enough and divisible by 4}
