...
\dontprintsemicolon
\SetKwFunction{redundant}{redundant}
\SetKwFunction{scdd}{scdd}
\SetKwFunction{linearity}{linearity}
\SetKwFunction{tyhull}{ty.hull}
\SetKwFunction{faceprop}{face.prop}
\SetKw{Sample}{Sample}
\SetKw{Estimate}{Estimate}
\SetKw{Calculate}{Calculate}
\SetKw{Find}{Find}
\SetKw{Question}{Question}
\SetKw{Set}{Set}

Get an initial value, $\eta_1$.  The observed data, $\yobs$, is of course known.\;
\Sample $Y_1, \ldots, Y_m$ from the distribution at $\eta_{1}$.\;
Make a V-rep from this sample and reduce using \redundant.  The result are the vertices of the convex hull. Store them in \tyhull.\;
\Estimate $\nabla \ell( \eta_1)$ and $\lVert \nabla \ell( \eta_1) \rVert$ using the sample.\;
\Set $p_1 = \nabla \ell( \eta_1)$. \;
\Set $k=2$, $LCM.k=1$, \texttt{LCM.flag=FALSE}. \;

\While{$\lVert \nabla \ell( \eta_k) \rVert > \epsilon$}{
\Find a step size $\alpha_k$ that satisfies the \textit{curvature condition}
\begin{align*}
	 0 & \leq \nabla \ell( \eta_k + \alpha_k p_k)^T p_k \leq c \nabla \ell(\eta_k)^T p_k
\end{align*}
for some $0 < c < 1$.  CJG thinks simulated tempering will be required to do this well.
\BlankLine
$\eta_{k+1} = \eta_k + \alpha_k p_k$.\;
\Sample $Y_1, \ldots, Y_m$ from the distribution at $\eta_{k+1}$.\;
Make a V-rep from this sample and reduce using \texttt{redundant}.\;
Combine reduced sample with \tyhull and apply \redundant again.\;
Get \textbf{H-rep} from reduced sample by using \scdd.\;
\BlankLine
\Question: Is $\yobs$ outside the convex hull of the sample?\;
\uIf{Yes, $\yobs$ is outside the convex hull}{  Keep sampling.  Skip to estimating $\nabla \ell( \eta_{k+1})$.
}
\uElseIf{No, $\yobs$ is inside the convex hull}{  Then the MLE exists, finding it should be straightforward.  Skip to estimating $\nabla \ell( \eta_{k+1})$.\\
}
\uElse{No, it's on the border.  In which case, either
\begin{enumerate}
\item the MLE exists but we got a sample that just touches $\yobs$, 
\item the MLE does not exist since $\yobs$ is itself on the border of the convex hull of the entire sample space.\;
\end{enumerate}
\Find the empirical face: apply the function \linearity on the rays formed from the cumulative convex hull relative, \tyhull, relative to $\yobs$.\;
\Calculate \faceprop, the proportion of the sample that falls on this face.

\uIf{$\faceprop < 10\%$}
{	Keep sampling!	Regardless of what case we're in.\;}
\uElse{
We've got $> 10\%$ of the sample falling on this face!\;
We assume that (1) above is not possible when we have 10\% of the sample on $\yobs$ or the estimated face.  So, we are in case (2): \textbf{the MLE does not exist.}\;

\BlankLine
\noindent \Question: how many points does \linearity return?\;
\uIf{only 1 point}
{We have either a single-point face. WHAT TO DO?\;
}
\uElse{
\textbf{multiple points}.\;
}
}
}
\BlankLine
