...
In the case of MCMC-MLE, it was shown in Section~\ref{S:MCMC-MLE} that getting close
enough to $g(\yobs)$ so that it is within the convex hull of the MCMC samples is a 
necessary condition for this algorithm to converge (in fact, this appears to be the
impetus for the steplength MCMC-MLE approach of \citep{Hummel}).  However, this is not
sufficient.  An effective approach may be to examine the importance sampling weights
used in \eqref{E:r_hat} in the previous iteration, and look for them to stabilize \citep{Geyer:1992}.
This ``rearview" approach should inform us if the previous $\eta_k$ was close 
enough to apply MCMC-MLE directly.
