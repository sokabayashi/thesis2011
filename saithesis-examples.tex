...
$\nabla^2 \ell(\eta)$ is useful when searching directly from MLEs;
...
model for which the MLE must exist.
...
\begin{align*}
	\delta = (6,-1)
\end{align*}
and is also a GDOR by definition.  The normal cone is derived by the methods described
in Chapter~\ref{Chapter:Linear programming}.
...
has returned the end points for one-sided confidence intervals in both natural
and mean value parameterization.
...
\begin{align*}
	\etaLCM = (94.0, -20.4 ).
\end{align*}
The normal cone to the observed statistic $N_C (g(\yobs) )$ in this case is two-dimensional, bounded by 
two vectors,
\begin{align*}
	 \{ (3.857,   -1),	(5.667,   -1) \}.
\end{align*}
...
Proceeding as before, then we find 95\% one-sided confidence intervals for $\eta$ of 
\begin{align*}
%14.495152 -3.703202 
	[14.5, +\infty)\\
	(-\infty, -3.7],
