...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=4.in,keepaspectratio]{pottsimage}
\end{center}
...
algorithm at 40 search directions; we did not attain our desired cutoff of 
...
$\tilde{F}$ is in the boundary of $C$.  If $\tilde{F}$ is in fact in the boundary of $C$, then 
by Theorem~\ref{Thm:LCM}, this threshold will be met.  A sample meeting this requirement 
is depicted in Figure~\ref{F:MC face}. 

%While 60\% is an arbitrary number, we do not want to set this threshold too high.
...
\includegraphics[width=5in]{Figures/MCsample-74face-BW}
...
The observed statistic, $(31,50)$, is the larger point on the boundary marked with a triangle.
A face has been identified,
defined by $(30,44)$, $(31,50)$, and $(32,56)$ and marked 
by triangles.  
Here, 74\% of the MC sample points fall on these three points.
The lighter colored polytope is the convex hull of all previously 
sampled points.% theta <- c(25,-4)
}
\label{F:MC face}
\end{figure}

By identifying the empirical face $\tilde{F}$ in whose relative interior the observed 
statistic lies, 
...
$s$ for which the the distribution indexed by $\etaLCM + \tilde{\gamma} + s \delta$ 
assigns a probability of 0.05 to the event that a sample falls on the 
face of interest.  That is, find $s$ such that
\begin{align*}
P_{\etaLCM + \tilde{\gamma} + s \delta}(g(Y) \in \tilde{F} ) = 0.05.
\end{align*}
...
second component of $\tilde{\delta}$ is negative.
...
The algorithm first identified a GDOR, $\tilde{\delta} = (6,-1)$, along which 
the log likelihood is strictly increasing.  It then used this empirical
GDOR to determine
a one-dimensional face $\tilde{F}$ in which the observed statistic lies 
in the relative interior.  The algorithm then found $\etaLCM$, the 
maximizer of the exponential family
which has $\tilde{F}$ as its convex support.  Finally, the algorithm used the
values of $\etaLCM$ and $\tilde{\delta}$ to calculate the end points for 
...
point $(27,27)$ is the empirical face $\tilde{F}$ and lies on the boundary of $C$, using the
...
example finding the MLE of this LCM,
\begin{align*}
	\etaLCM = (35.38, -9.39),
\end{align*}
...
If the end goal is to try to interpret these numbers by their sign and  
...
$\tilde{\delta} = (4,-1)$, which is also a direction of constancy for the LCM. 
