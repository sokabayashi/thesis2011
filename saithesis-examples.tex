...
\begin{table}
\caption[Comparison of step sizes for SA and our algorithm for Example 1]{The first 20 step sizes used by  SA (with step size $1/k$) and our algorithm 
...
$\eta = \left(0, \log(1 + \sqrt{2}) \right)^T$]
{A realized sample from an Ising model on a $32 \times 32$ lattice with 
$\eta = \left(0, \log(1 + \sqrt{2}) \right)^T$.  This value of $\eta$ corresponds to the phase transition point, where 
...
\begin{table}
\caption[Comparison of MLEs for $\eta$ for Example 2]{Comparison of MLEs for $\eta$ for Example 2: MLE = Newton-Raphson starting 
...
\end{center}
\caption[Histogram of the $\eta$ component for 1000 MLEs for an Ising model on a $32 \times 32$ lattice with $\eta = \left(0, \log(1 + \sqrt{2}) \right)^T$]
...
in Chapter~\ref{Chapter:Linear programming} which require first calculating a GDOR, 
\begin{align*}
	\delta = (6,-1).
\end{align*}
(This GDOR can also be used as an effective search direction 
in the same manner as the regression direction described in Section~\ref{S:Search directions}.)

The second task turns out to be very difficult to do.  
