...
back $q$ to the rays will return the original extreme points.
...
or boundary of the hull.  The \texttt{linearity} function applied to the extreme points of a hull identify empirically a face $F$ on which $q$ lies.  
...
points of a face on the boundary of the hull.  By construction, $q$ lies in the
the relative interior of this face.
...
We are often interested in the normal cone and tangent cones after it has been
determined that $q$ is on the boundary of a  hull.  
...
\end{align*}
We apply this to the point (21,13) on the same hull as we have been using,
where there are no equality constraints:
\begin{verbatim}
> q <- as.matrix( c("21","13") )
> sample.gy.Hrep
$output
