...
%\newpage
\section{Algorithm pseudocode extension} \label{S:pseudocode new}
...
\If{$LCM.flag=$TRUE}
	\If{Any new points refute claim that $\tilde{F}$ is a face}
...
%\Else 
	...
	\State $g(\yobs) \in \rbd( gy.hull )$ and $g(\yobs) \in \rint \tilde{F}$
		...
		\If{$>60\%$ of the sample points are in $\tilde{F}$ (via $\tilde{\delta}$)}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=5.6in]{combined-hull-norm}
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Interior and boundary point}
...
%face $F$.  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Normal and tangent cones} \label{S:ncone tcone}
...
If in our algorithm we find a new sample point $w$ for which 
