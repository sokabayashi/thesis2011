...
in this process
including Besag's \emph{pseudo likelihood} approach \citep{Besag:1974,Strauss:1990}, 
...
restricted earlier models like $p_1$ and $p^*$ to their simpler settings.
...
In this paper, we consider the ability to simulate such networks the end goal of parameter
...
relations 
exist $i \to j$ and $j \to i$, or \emph{transitive} relations, where 
relations exist $i \to j$, $j \to k$, $i \to k$.  The vector $g(y)$ can then be defined 
\begin{align*}
...
same ``type" tend to make more relations with others of that type.  
