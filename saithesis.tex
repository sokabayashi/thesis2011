...
estimators (MPLE)} obtained from a standard logistic regression (we will discuss the pseudolikelihood approach in Section~\ref{S:pseudolikelihood}) and observe that the 
...
maximize $PL(\eta)$.  These values of $\eta$ are called the maximum pseudolikelihood estimators (MPLE).
...
to the pseudolikelihood one except that the conditional distributions used to build the \emph{composite likelihood function}---analogous to the pseudolikelihood function---are for multiple dyads rather than a single dyad.
...
For this value of $\eta$, the model dictates that each of the 306 possible edges will occur independently with probability $p = \frac{1}{1+e^{-\eta}} = \frac{1}{1+e^{-1}} = 0.731$.   
...
\begin{table}[h] 
\caption{Appeal and drawbacks of various parameter estimation methods. MPLE=Maximum pseudolikelihood estimator,
...
\section{Non-existent MLEs in exponential families} \label{S:Non-exist MLE}
...
may not exist.  The theoretical background 
...
\citet{Geyer:1992} separated the search for the MLE an exponential family model
with complex dependence into a two-phase 
...
this approach.
In the case of generalized linear models including logistic regression and contingency 
...
does exist and can be found using conventional methods such as logistic regression.  
...
\begin{enumerate}
\item The MLE does not exist.  
\item The MLE is difficult to obtain, including cases where MCMC methods fail to converge
...
%
The issue is serious enough to be a barrier to progress in the field \citep{advancesp*}.  
...
Adolescent Health\footnote{The data set, which \citeauthor{statnet-tutorial} refer to as Faux 
...
possible to explicitly evaluate the likelihood function \eqref{E:ERGM} and thus find MLEs
using standard numerical optimization routines.  As in \citep{Geyer:gdor},
the emphasis is on the geometric properties of the polyhedral convex support of the model;
the support for the 9-node ERGM with a two-dimensional network statistic of
edges and triangles
studied by \citeauthor{Rinaldo:2009} is depicted in Figure~\ref{F:g9-hull}.
\citeauthor{Rinaldo:2009} illustrate how the normal fans of this support 
...
geometry of this convex support, only attainable by enumerating all possible graph 
realizations.  In addition, while they dissect these particular examples 
to explain the problematic behavior, they recommend no general methodology 
to detect nor cope with non-exist MLEs.
...
\begin{enumerate}
	\item avoiding the hassle of trial-and-error algorithm calibration
	\item being insensitive to starting point
\end{enumerate}
\item When the MLE does not exist, the algorithm finds the MLE in the Barndorff-Nielsen completion and the 
accompanying \emph{direction of recession} necessary to build one-sided confidence intervals 
in the manner of \citep{Geyer:gdor}.
\end{enumerate}

In short, our proposed algorithm addresses many of the drawbacks of the algorithms
described in Section~\ref{S:ERGM examples} as well as providing a general approach
to dealing with non-existent MLEs.

\subsection{some details ...}

 and is unique, and the first derivative of the log likelihood 
can be calculated exactly.
When the first derivative of the log likelihood can be calculated exactly, 
it still has a particularly convenient form that is 
easily estimable with MCMC, 
making the algorithm still useful in application.
  
We also show how to construct and 
...
\citep{Goldenberg:2009} for an comprehensive introduction to social network modeling 
