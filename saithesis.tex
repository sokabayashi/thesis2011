...
%\input{Introduction}
\section{Motivation: stochastic social network models}
...
This is the question that led us to do research in parameter estimation methods for exponential families, with a particular focus on social network models.  Sociologists have studied social networks as a subject matter since at least as early as the 1930s, when Moreno introduced the sociogram, a diagram that denotes individuals as points, and the presence of a relationship between individuals by lines \citep{Wasserman:1994}.  A sociogram depicting the marriage network data among important families in Renaissance Florence presented in \citep{Padgett} is depicted in Figure~\ref{F:Florentine}.
\begin{figure}
\begin{center}
\includegraphics[width=4.5in,keepaspectratio]{florentine}
\end{center}
\caption{\citet{Padgett}'s marriage network among families in Renaissance in the late 15th century.  Data made available in the \texttt{ergm} package \citep{ergm} in R.}
\label{F:Florentine}
\end{figure}

\emph{Stochastic} network models were developed as early as 1959 in the seminal works of \citet{Gilbert} and \citet{Erdos}, resulting in a basic model that is now referred to as the Bernoulli model or the Erd\"{o}s-R\'{e}nyi-Gilbert model.  With the advent of cheaper computing resources, the development of more sophisticated stochastic network models flourished;  notable landmarks are the $p_1$ model of \citet{Holland:1981}, which could capture the reciprocal tendency of relationships in directed networks, and the $p^*$ model of \citet{Frank:1986}, which could describe transitive tendency of relationships in undirected networks.  These models laid the foundation for the more general \emph{exponential random graph models (ERGM)}, a class of exponential family models that are now commonly used to model networks \citep{logit,Pattison:1999,Snijders:2002,Handcock:2006,introp*,advancesp*,recentp*,ergm,Morris:2008,Goldenberg:2009}.

\subsection{THE ISSUE -- DEPENDENCE}
At the core, a stochastic network model attempts to describe whether or not a relationship forms between each pair of individuals in a collection of individuals.
%  In the Florentine marriage network depicted in Figure~\ref{Padgett}, a model fit  
It has long been observed though that relationships do not form in isolation; rather, they form in an interdependent manner.  Whether or not a relationship forms between individuals $A$ and $B$ may very well depend on whether or not a relationship forms between $B$ and $C$.  

This necessitates a new ``network" perspective that recognizes relationship structures themselves as factors in the analysis.  

\subsection{MICRO -- MACRO}

\subsection{PARAMETER ESTIMATION}
Writing down a mathematical model to describe data is often not difficult to do; the expression $y=mx+b$ should be familiar to anyone who has taken basic algebra as the equation of a line.  However, finding a ``best fit" line through scattered data points---that is, finding appropriate values for the \emph{parameters} $m$ and $b$ is a much more involved mathematical exercise that involves calculus.

Similarly, exponential families---and hence exponential random graph models---have a deceptively simple form that belies the challenges involved in the estimating the model parameters.
...
\section{Exponential family modeling}
VS. ERGM?
