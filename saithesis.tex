...
the difficulties
result from the normalizing constant \eqref{E:kappa} which may involve 
a summation over an astronomical number of terms as discussed 
in Section~\ref{S:intractable}.
...
and has become the standard approach in the  
literature \citep{Hunter:2006, Handcock:2006, GOF} and is the default algorithm in 
...
\begin{align}
 r( \eta, \eta^0 ) &= \ell( \eta ) - \ell( \eta^0 ) \notag \\ 
				  &= ( \eta - \eta^0)^T g(\yobs) - \log \left [ \exp \bigl( c(\eta) - c(\eta^0) \bigr) \right ].\label{E:r}
\end{align}
...
extremely difficult for the algorithm to attain the MLE in a single iteration.  
...
to $\eta$ shows that if the MCMC engine is unable to generate any $g(Y)< g(\yobs)$, 
\eqref{E:r_hat} will be strictly negative, resulting in $\hat{r}(\eta,\eta^0)$ with no
maximum, as depicted by the dotted-line in Figure \ref{F:MCMC-MLE} (left).  
The problem is not present when the initial value is close to the true MLE such as 
$\eta^0 = -1$; the corresponding $r(\eta,\eta^0)$ and $\hat{r}(\eta,\eta^0)$ are
depicted in Figure \ref{F:MCMC-MLE} (right).  With the default three 
...
It should be noted that improvements to the MCMC-MLE approach is an active area of research \citep{Hummel}. 
