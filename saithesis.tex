...
to determine which fits the data best.  The authors also get \emph{maximum pseudo likelihood 
estimators (MPLE)} obtained from a standard logistic regression (we will discuss the pseudo likelihood approach in the next section) and observe that the 
...
{E:ERGM} that belies the challenges involved in parameter estimation,
resulting from the intractable normalizing constant discussed in Section~\ref{S:intractable}.
...
that circumvent evaluating the likelihood function.
